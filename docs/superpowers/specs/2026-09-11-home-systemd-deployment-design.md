...
相对数据和日志路径继续由 `config.yaml` 控制；README 使用上面的默认示例。
...
README 改为在 `/home/ubuntu/usd1_monitor` 中创建虚拟环境、复制示例配置和 `.env`、创建 `data/` 与 `logs/`，并以 `ubuntu` 用户执行备份和清理命令。

更新部署示例测试，断言两个 unit 使用新的用户、路径和读写边界。除部署路径外，不修改数据库结构、应用配置模型、采集频率或程序行为。
