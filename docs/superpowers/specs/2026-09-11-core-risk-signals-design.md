...
- 现有完整区块读取结果提供区块真实时间；转账窗口不使用程序扫到事件的时间。
...
- 少量安全头或边界读取及余量：约 15,000 响应/月；
- 总新增约 140,000–160,000 响应/月；
- 按失败重试两倍预留仍低于 350,000 响应/月。

EVM 资金流复用已有日志和完整区块读取，不重复拉取 Transfer 日志。普通 HTTPS 不计入 Dwellir。按 Dwellir Developer 每月 25M responses 计算，本功能约占 0.6%，双倍预留约占 1.4%。
